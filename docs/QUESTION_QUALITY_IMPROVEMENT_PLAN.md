...
> **Phase 2K-Improve-B4 (2026-05-25):** All 26 identifiable PR IMPROVE items resolved. Improvements applied across all 13 chapters using 23 distinct distractor trap types. `prdra`, `prdrb`, `prb200` payloads rebuilt and re-baked. 0 validation errors. See `docs/PHASE_2K_IMPROVE_B4_PR_LOG.md`.

> **Phase 2K-Improve-B5 (2026-05-25):** All 27 non-Public IMPROVE questions in Generated Barrister C (13) and F (14) improved across Civil Litigation, Criminal Law, and Family Law. 1 correct answer changed (civ-02-basic-lim-001: A→B, to support delayed-discovery redesign). 26 new distractor trap types introduced. `barc` payload rebuilt (398,740 → 413,636 b64 chars); `barf` payload rebuilt (506,124 → 513,328 b64 chars). All other payloads unchanged. 963 questions, 0 validation errors. C∩D=0, C∩E=0, C∩F=0, D∩E=0, D∩F=0, E∩F=0, C∪D∪E∪F=640 unique IDs. See `docs/PHASE_2K_IMPROVE_B5_CF_NONPUBLIC_LOG.md`.

> **Phase 2W — Validator Warning Reduction (2026-05-25):** Reduced validator warnings from 798 to **123** (85% reduction, 0 errors). Three categories of safe style fixes: (1) 610 explanation openings transformed to start with "Under [statute/rule]..." using 13 pattern-based strategies (direct statute prepend, Section→s., The-statute→Under-the-statute, SR-prefix for the rest); (2) 56 `estimated_time_seconds` values raised to the minimum valid for their difficulty level (hard→120s, exam_trap→150s, medium→90s); (3) 9 PR source_ref plural-rules citations fixed ("Rules X.X-Y, X.X-Z" → "Rule X.X-Y (see also Rules X.X-Z)"). All remaining 123 warnings are ACCEPT: 47 case-law-only explanation contexts + 76 case-law/By-Law/Charter source_refs with no applicable statute section. All four generated barrister payloads rebuilt (barc 413,636→417,840; bard→456,836; bare→537,976; barf 513,328→517,940); PR payloads rebuilt (prdra→303,232; prdrb→296,996; prb200→600,372). HTML validation: all checks pass. Integrity: C=D=E=F=160, C∩F=0, C∪D∪E∪F=640, PRA=100, PRB=100, PR200=200. Fix scripts: `tools/fix_2w_pr_rules_plural.py`, `tools/fix_2w_time_difficulty.py`, `tools/fix_2w_explanation_format.py`. Rebuild script updated to include bard+bare. See `docs/PHASE_2W_WARNING_AUDIT.md` and `docs/PHASE_2W_WARNING_REDUCTION_LOG.md`.

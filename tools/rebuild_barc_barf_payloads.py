...
def load_all_questions() -> dict:
    """Return {id: (question_dict, section_name)} from all question files."""
    bank = {}
    for qdir in QUESTION_DIRS:
        sec = SEC_MAP[qdir.name]
...
def to_compact(q: dict, sec: str, seq: int) -> dict:
...
def build_b64(manifest_path: Path, bank: dict) -> tuple[str, int]:
    ...
    for seq, qid in enumerate(ids, start=1):
        if qid not in bank:
            ...
            continue
        q, sec = bank[qid]
        compact.append(to_compact(q, sec, seq))
    if missing:
